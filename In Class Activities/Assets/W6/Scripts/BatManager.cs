...
public class BatManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            float distance = Vector3.Distance(_playerTransform.transform.position, _bats[i].transform.position);
            if (distance < _interactDistance)
            {
                _bats[i].GetComponent<BatW6>().StartChase();
                Debug.Log("interact"+ distance + " " + i);
                if (distance < _overlapDistance)
                {
                    Debug.Log("overlap"+ distance + " " + i);
                    CreateReactions(_bats[i].GetComponent<BatW6>());
                }
            }
            else
            {
                _bats[i].GetComponent<BatW6>().StopChase();
            }
        }
    ...
    private void CreateReactions(BatW6 bat)
    {
        ...
        //      of CreateReactions.
        string msg = _messages[Random.Range(0, _messages.Length-1)];
        SpawnReactionUI(bat, msg);
    ...
    private void SpawnReactionUI(BatW6 bat, string message)
    {
        ...
        int index = System.Array.IndexOf(_bats, bat.gameObject);
